...
.controller('AccountCtrl', function($scope,$rootScope,$timeout,$cordovaNetwork,$state,AuthService,Account,$http,$location) {
  $scope.data = {};
  $scope.data.staffName = '';
  $scope.data.affiliation = '';
  $scope.data.divisionName = '';
  $scope.data.areaName = '';
  $scope.data.sectionName = '';
  ...
  var getProfile_ = function(yyyymm){
    Account.get(yyyymm).then(function(response){
      $scope.data = response.data;
    }),function(error){};
  };
  ...
  $scope.login=function(data){
    ...
    ApiService.Post('users/mobilelogin/',{"User":{"loginid":data.loginid,"password":data.password}}).then(function(response){
      ...
      if(response['user']["User"]){
        ...
        AuthService.storeUserCredentials(token,response['user']["Profile"]['name'],response['user']["User"]['id'],response['user']["User"]['outside_userid']);
