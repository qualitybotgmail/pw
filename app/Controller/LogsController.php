 ...
 */
	public function add() {
		...
		$threads = $this->Log->Thread->find('list');
		$heads = $this->Log->Head->find('list');
		$messages = $this->Log->Message->find('list');
		$comments = $this->Log->Comment->find('list');
		$likes = $this->Log->Like->find('list');
		$users = $this->Log->User->find('list');
		$this->set(compact('users', 'threads', 'heads', 'messages', 'comments', 'likes', 'users'));
	}
